 ...
 *
 * @WebformHandler(
 *   id = "cat_a_certificate_handler",
 *   label = @Translation("Email CAT A Cert"),
 ...
 * )
 */
class CatACertificateHandler extends WebformHandlerBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    ...
    if ($_POST['op'] == 'Submit') {
      ...
      // Find attendee by email & conf year ID
      $email = $values['attendee_email_year'];
      $config = \Drupal::config('atsnj_ebp.settings');
      $ebp_current_year = $config->get('current_year');
      $attendee = _atsnj_load_attendees_by_email($email, FALSE, $ebp_current_year);
      ...
      $module = 'atsnj_ebp';
      $key = 'send_cat_a_certificate_mail';
      $to = $email;
      $params = [];
      ...
      // Load the Cert
      $cert_ids = $attendee->get('field_cat_a_certificate')->getValue();
      ...
      if (!$cert) {
        ...
        _atsnj_bulk_actions_make_cat_a_cert($attendee);
      ...
      if (!$cert) {
        ...
        // Add the PDF
        $file_id = $cert->get('field_media_file_3')->getString();
